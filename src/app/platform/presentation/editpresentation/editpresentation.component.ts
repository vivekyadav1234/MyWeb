...
@Component({
  selector: 'app-editpresentation',
  templateUrl: './editpresentation.component.html',
  styleUrls: ['./editpresentation.component.css'],
...
export class EditpresentationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    var prod = localStorage.getItem("used_product");
    if(prod){
      this.added_products = JSON.parse(prod);
    }
  }
  ngAfterViewInit(){
    this.getSections();
  ...
  fetchPresentation(){
    ...
    this.presentationService.fetchPresentation(this.project_id,this.presentation_id).subscribe(
      presentation => {
        ...
        this.lead_id = this.presentation.presentation.lead_id
        this.fetchPptProducts();
  ...
  fetchPptProducts(){
    ...
    this.presentationService.fetchPptProducts(this.project_id,this.presentation_id).subscribe(
      ppt_products => {
        this.ppt_products = ppt_products.presentations_products

        // for (let product of this.ppt_products) {
        //   var obj = {
  ...
  createBOQ(){
    ...
    this.presentationService.saveBoq(products_used,this.project_id, this.presentation.presentation, this.boq)
    .subscribe(
        boq => {
          boq = boq;
  ...
  saveFile(){
    this.loaderService.display(true);
    var data = this.canvasMap;
    var products_used = this.added_products;
    var title = $("#p-title").val();
    this.presentationService.savePresentation(data,products_used,this.project_id, this.presentation.presentation, title)
    .subscribe(
        presentation => {
  ...
  // linked_products:any;
  linkedProducts(){
    $("#linkedProductModal").modal("show");

    // this.presentationService.linkedProducts(products_used,this.project_id, this.presentation.presentation, this.boq)
    // .subscribe(
    //     products => {
    //       this.linked_products = products;
    //       // Object.keys(boq).map((key)=>{ this.boq= boq[key];});
    //       // 
    //       this.loaderService.display(false);
    //     },
  ...
  deleteProducts(product_id){
    ...
    this.presentationService.deleteProducts(this.project_id,this.presentation_id,product_id ).subscribe(
      ppt_products => {
        this.fetchPptProducts();
      },
      err => {

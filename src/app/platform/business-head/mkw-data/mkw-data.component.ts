...
@Component({
  selector: 'app-mkw-data',
  templateUrl: './mkw-data.component.html',
  styleUrls: ['./mkw-data.component.css'],
  providers:[BusinessHeadService]
})
export class MkwDataComponent implements OnInit {
  ...
  getFHIData(){
    ...
    this.businessHeadService.getFHIAndMKWData('mkw_data',this.filter_by_type,this.from_date,this.to_date,this.date_filter_type,this.cm_list,this.designer_list,this.city).subscribe(
      res => {
  ...
  getCMsAndDesForCity(cityId?){
    this.loaderService.display(true);
    this.businessHeadService.getCMsAndDesForCityForOther('mkw_data',cityId).subscribe(
      res => {

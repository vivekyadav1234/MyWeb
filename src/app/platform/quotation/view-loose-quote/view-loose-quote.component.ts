...
@Component({
  ...
  providers: [QuotationService,LeadService,CatalogueService]
})
export class ViewLooseQuoteComponent implements OnInit {
  ...
  addAddonForAddons(val,val2?,index?){
    let getFun = (<FormArray>(<FormArray>this.customizationModule_MK_Form.get('compulsory_addons')).controls[index]).controls['addons_for_addons'];
  ...
  getGlobalVariable(category,arrivaeSelectValues){
    ...
    this.loaderService.display(true);
    this.quotationService.getGlobalVariable(category,arrivaeSelectValues).subscribe(
      res=>{
  ...
  quotationsId;
  viewImportQuotationId(){
    this.quotationService.viewImportQuotationDetails(this.projectId,this.qid).subscribe(
      res=>{
  ...
  createSpace(formVal,category){
    if(formVal.space!=""){
      if(category=='wardrobe' && !this.selectedSpacesArr.includes(formVal.space)){
        ...
        this.quotationService.setSpaces(formVal.space,this.projectId,this.qid,'custom_furniture').subscribe(
          res =>{
            this.loaderService.display(false);
            this.quotationDetails = res.quotation;
            this.selectedSpacesArrCf = res.quotation.spaces_custom_furniture;
        ...
        obj ={
          "product_module":{
            ...
            "options": {
              ...
              "hardware_brand_id": (this.quotations.boq_global_configs[0].brand)?this.quotations.boq_global_configs[0].brand.id:'',
              "addons": [],
              "custom_elements": []
            }
          }
  ...
  updateQuantity(rowno,mod,secname){
      ...
      this.quotationService.updateQuantity(this.projectId,this.qid,obj2,secname).subscribe(
        res=>{
          this.quotations = res.quotation;
          this.ref.detectChanges();
          // this.editModuleLineQty = undefined;
  ...
  addProductToBoqs(product,quantity){
    ...
    this.quotationService.addboqJobToSpace(this.projectId,this.qid,obj).subscribe(
      res=>{
        this.quotations = res.quotation;
        this.loaderService.display(false);
        this.product_notify_message = product.name + ' has been added';
  ...
  removeProductToBoqs(productid,productname){
    if(confirm('Are you sure you want to delete this product?')==true){
      ...
      this.quotationService.deleteboqJobToSpace(this.projectId,this.qid,obj).subscribe(
        res=>{
            this.quotations = res.quotation;
  ...
  updateProduct(rowno,mod){
    ...
    this.quotationService.updateboqJobOfSpace(this.projectId,this.qid,obj2).subscribe(
      res=>{
        this.quotations = res.quotation;
    ...
    var data = {
      ...
      this.quotationService.updateBOQData(this.qid,this.projectId,data).subscribe(
        res=>{
          ...
          this.successMessageShow((this.quotations.handover_present ? 'BOQ updated successfully, Kindly click on Update & Handover Quotation button to handover BOQ' : 'BOQ updated successfully'));
  ...
  updateHandoverQuotation(){
    ...
    this.quotationService.updateHandoverBOQData(this.qid,this.projectId,data,this.quotations.handover_present).subscribe(
      res=>{
        this.boqServices_all=[];
        this.boqServices = [];
        this.selectedSections.length=0;
        ...
        this.successMessageShow(this.quotationService.updateBoqSuccessMessage);
        this.editBtnFlag = false;
        this.loaderService.display(false);
      },
      err=> {
...
onCheckChangeFilter(event,fileterField,fileterFor){
  ...
  nextPage(pageFor,addontype?,category?){
    if(pageFor=='extra'){
      ...
      if(addontype =='compaddon'){
        ...
        this.getListofAddonsForAddons(this.getformArray(this.customizationModule_MK_Form,'compulsory_addons').controls[this.slotindex]['controls'].id.value,this.getformArray(this.customizationModule_MK_Form,'compulsory_addons').controls[this.slotindex]['controls'].combination.value,
          this.getformArray(this.customizationModule_MK_Form,'compulsory_addons').controls[this.slotindex]['controls'].slot.value,this.slotindex);
      }else if(addontype =='optionaladdon'){
        this.getOptionalAddons(category)
      }
    } 
  ...
  filterProducts(page?){
    ...
    this.loaderService.display(true);
      ...
      this.checked_finishes, this.checked_subcategories, this.checked_minimum_price,
       ...
       this.checked_minimum_height,this.checked_maximum_height,this.new,page).subscribe(
      res => {
        this.headers_res= res.headers._headers;
        this.per_page = this.headers_res.get('x-per-page');
        this.total_page = this.headers_res.get('x-total');
  ...
  savePresetAsGlobal(category,presetId){
    this.loaderService.display(true);
    this.quotationService.savePresetAsGlobal(category,presetId,this.qid).subscribe(
      res=>{
        this.getBoqConfig(category);
    ...
    if(formval.category=='kitchen'){
      ...
      for(var i=0;i<keys_arr.length;i++){
        if(keys_arr[i]==formval.space){
          for(var j=0;j<this.quotations.modular_jobs_kitchen[keys_arr[i]].length;j++){
            mod_job_ids.push(this.quotations.modular_jobs_kitchen[keys_arr[i]][j].id);
          }
        }
      }
      for(var i1=0;i1<keys_arr1.length;i1++){
        if(keys_arr1[i1]==formval.space){
          for(var j1=0;j1<this.quotations.appliance_jobs[keys_arr1[i1]].length;j1++){
            appliance_job_ids.push(this.quotations.appliance_jobs[keys_arr1[i1]][j1].id);
          }
        }
      }
      for(var i2=0;i2<keys_arr2.length;i2++){
        if(keys_arr2[i2]==formval.space){
          for(var j2=0;j2<this.quotations.extra_jobs_kitchen[keys_arr2[i2]].length;j2++){
            extra_job_ids.push(this.quotations.extra_jobs_kitchen[keys_arr2[i2]][j2].id);
          }
        }
      }
      ...
      for(var i3=0;i3<keys_arr.length;i3++){
        if(keys_arr[i3]==formval.space){
          for(var j3=0;j3<this.quotations.modular_jobs_wardrobe[keys_arr[i3]].length;j3++){
            mod_job_ids.push(this.quotations.modular_jobs_wardrobe[keys_arr[i3]][j3].id);
          }
        }
      }
      for(var i2=0;i2<keys_arr2.length;i2++){
        if(keys_arr2[i2]==formval.space){
          for(var j2=0;j2<this.quotations.extra_jobs_wardrobe[keys_arr2[i2]].length;j2++){
            extra_job_ids.push(this.quotations.extra_jobs_wardrobe[keys_arr2[i2]][j2].id);
          }
        }
      }
    }
  ...
  importLayoutToSpace(layoutid){
    this.loaderService.display(true);
    this.quotationService.importMkwLayout(this.projectId,this.qid,
  ...
  generateSmartQuotation(data){
    this.loaderService.display(true);
    this.quotationService.generateSmartQuotation(this.projectId,this.qid,data).subscribe(
      res=>{
        this.quotations=res.quotation;
  ...
  setCombinationElement(event,idRef,elementIndex,addOnsLength,addonIndex){
    ...
    if($(`#${idRef}-${addonIndex}-${elementIndex}`)){ 
      ...
      $(`#${idRef}-${addonIndex}-${elementIndex}`).prop("checked", true).change();
      this.ref.detectChanges;
    }
  ...
  uploadCfExcel(space,index){
    ...
    this.quotationService.uploadCfExcel(this.projectId,this.qid,this.basefile,space,this.file_name).subscribe(
      res=>{
        this.quotationDetails=res.quotation;
        $('#uploadInput'+index).val('');
  ...
  importCFLayoutToSpace(id){
    this.loaderService.display(true);
    this.quotationService.importCFLayoutToSpace(this.projectId,this.qid,this.custom_furniture_space,id).subscribe(
      res=>{
        this.quotationDetails=res.quotation;
  ...
  removeCfProductToBoqs(id){
    this.cfIds = id;
    this.loaderService.display(true);
    this.quotationService.deleteCustomFurnitureLayout(this.projectId,this.qid,this.cfIds).subscribe(
      res => {
        this.quotationDetails=res.quotation;
        this.successalert = true;
  ...
  deleteLabel(labelIndex,id){
    this.loaderService.display(true);
    this.quotationService.deleteLabelOfLineItem(this.projectId,this.qid,id).subscribe(
      res=>{
        ...
        if(this.selectedsectionName == 'modular_kitchen' ){
          ...
          if(this.ownerableType=="ModularJob"){
            this.quotations.modular_jobs_kitchen[this.labelKey][this.indexofType]=JSON.parse(res._body).modular_job;
            this.labelList=this.quotations.modular_jobs_kitchen[this.labelKey][this.indexofType].labels
          }
          if(this.ownerableType=="ApplianceJob"){
            this.quotations.appliance_jobs[this.labelKey][this.indexofType]=JSON.parse(res._body).appliance_job;
            this.labelList=this.quotations.appliance_jobs[this.labelKey][this.indexofType].labels  
          }
          if(this.ownerableType=="ExtraJob"){
          this.quotations.extra_jobs_kitchen[this.labelKey][this.indexofType]=JSON.parse(res._body).extra_job;
          this.labelList=this.quotations.extra_jobs_kitchen[this.labelKey][this.indexofType].labels  
          }    
        }
        if(this.selectedsectionName == 'modular_wardrobe' ){
          if(this.isCombined){
            if(this.ownerableType=="ModularJob"){
              this.quotations.modular_jobs_wardrobe[this.labelKey][this.indexofType].included_modules[this.combinedIndex]=JSON.parse(res._body).modular_job;
              this.labelList=this.quotations.modular_jobs_wardrobe[this.labelKey][this.indexofType].included_modules[this.combinedIndex].labels;
            }
          }
          else{
            if(this.ownerableType=="ModularJob"){
              this.quotations.modular_jobs_wardrobe[this.labelKey][this.indexofType]=JSON.parse(res._body).modular_job;
              this.labelList=this.quotations.modular_jobs_wardrobe[this.labelKey][this.indexofType].labels;
        ...
        if(this.selectedsectionName == 'loose_furniture' ){
          if(this.ownerableType=="Boqjob"){
            this.quotations.boqjobs[this.labelKey][this.indexofType]=JSON.parse(res._body).boqjob;
            this.labelList=this.quotations.boqjobs[this.labelKey][this.indexofType].labels 
          } 
        }
        if(this.selectedsectionName == 'custom_elements' ){
          if(this.ownerableType=="CustomJob"){
            this.quotations.custom_jobs[this.labelKey][this.indexofType]=JSON.parse(res._body).custom_job;
            this.labelList=this.quotations.custom_jobs[this.labelKey][this.indexofType].labels 
          } 
        }
        if(this.selectedsectionName == 'custom_furniture' ){
          if(this.ownerableType=="ShangpinJob"){
            if(this.shangpinType=='door'){
              this.quotations.shangpin_jobs_door[this.labelKey][this.indexofType]=JSON.parse(res._body).shangpin_job;
              this.labelList=this.quotations.shangpin_jobs_door[this.labelKey][this.indexofType].labels  
            }
            if(this.shangpinType=='cabinet'){
              this.quotations.shangpin_jobs_cabinet[this.labelKey][this.indexofType]=JSON.parse(res._body).shangpin_job;
              this.labelList=this.quotations.shangpin_jobs_cabinet[this.labelKey][this.indexofType].labels  
            }
            if(this.shangpinType=='accessory'){
              this.quotations.shangpin_jobs_accessory[this.labelKey][this.indexofType]=JSON.parse(res._body).shangpin_job;
              this.labelList=this.quotations.shangpin_jobs_accessory[this.labelKey][this.indexofType].labels  
            }
            if(this.shangpinType=='slidingDoor'){
              this.quotations.shangpin_jobs_sliding_door[this.labelKey][this.indexofType]=JSON.parse(res._body).shangpin_job;
              this.labelList=this.quotations.shangpin_jobs_sliding_door[this.labelKey][this.indexofType].labels  
            }
            if(this.shangpinType=='wardrobe'){
              this.quotations.shangpin_jobs_wardrobe[this.labelKey][this.indexofType]=JSON.parse(res._body).shangpin_job;
              this.labelList=this.quotations.shangpin_jobs_wardrobe[this.labelKey][this.indexofType].labels  
            }
          }     
        }
...
editLabel(labelIndex,labelName,id){
  this.loaderService.display(true);
  this.quotationService.editLabelOfLineItem(this.projectId,this.qid,id,labelName).subscribe(
    res=>{
      ...
      if(this.selectedsectionName == 'modular_wardrobe' ){
        if(this.isCombined){
          ...
          if(this.ownerableType=="ModularJob"){
            ...
            this.quotations.extra_jobs_wardrobe[this.labelKey][this.indexofType]=JSON.parse(res._body).extra_job;
            this.labelList=this.quotations.extra_jobs_wardrobe[this.labelKey][this.indexofType].labels  
          }   
        }
...
addLabel(value){
  this.loaderService.display(true);
  this.quotationService.addLabelToLineItem(this.projectId,this.qid,this.ownerableType,this.ownerableId,value).subscribe(
    res=>{

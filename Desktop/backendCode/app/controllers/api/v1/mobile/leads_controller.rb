...
require "#{Rails.root.join('app','serializers','lead_serializer')}"

class Api::V1::Mobile::LeadsController < Api::V1::ApiController
  ...
  before_action :set_lead, only: [:show, :update, :destroy, :approve_user, :update_status, :mark_escalated, 
    :get_lead_info, :assign_cm_to_lead, :update_basic_info, :alternate_contacts, :sales_life_cycle_report,
    :mark_as_read
  ]
  ...
  def get_pincodes_on_city_basis
   if params[:city].present?
     ...
     render json: @zipcodes_codes, status: 200
   end
  end
  ...
  def mark_as_read
    @lead.mark_as_read
    render json: @lead, serializer: MobileLeadSerializer
  end

  # GET /api/v1/mobile/leads
  def index
    # begin
      allowed_status = ['qualified']
      if params.dig(:is_in_pipeline) 
        # unless (params.dig(:is_in_pipeline).downcase == true || 
        #   params.dig(:is_in_pipeline).downcase == "true")
          allowed_status = ['qualified', 'follow_up', 'wip', 'not_contactable']
        # end
      end
      if current_user.has_any_role?(:designer)
        @leads = Lead.where(
          id: current_user.designer_projects.where(active: true).pluck(:lead_id),
        ).where(
          id: Project.where(status: allowed_status).pluck(:lead_id),
        ).where(lead_status: allowed_status)
      else
        ...
        lead_ids = Lead.joins(:project).joins(:assigned_cm).where(
          id: Project.where(status: allowed_status).pluck(:lead_id)).where(
          users: {id: cms}).where(lead_status: allowed_status).pluck(:id)
        qualified_leads = DesignerProject.where(active: true, lead_id: lead_ids).pluck(:lead_id)
        @leads = Lead.where(id: qualified_leads)

      end
      pipeline = @leads.where(is_in_pipeline: true).pluck(:id) + @leads.nil_pipeline_items.pluck(:id)
      leads_in_pipeline = pipeline.uniq.count
      active_leads = (@leads.pluck(:id).uniq - pipeline.uniq).uniq.count
      if params.dig(:is_in_pipeline)
        if params.dig(:is_in_pipeline).downcase == true || params.dig(:is_in_pipeline).downcase == "true"
          # p "Is in pipeline : True"
          @leads = Lead.where(id: pipeline)
        else
          # p "Is in pipeline : False"
          @leads = Lead.where(id: @leads.pluck(:id) - pipeline)
        end
      end
      @leads = Lead.where_with_order(:id, @leads.joins(
          project: :designer_projects
        ).where(
          designer_projects: {active: true}
        ).order("designer_projects.updated_at DESC").pluck(:id))
      hash_to_render = ActiveModelSerializers::SerializableResource.new(
        @leads, each_serializer: MobileLeadSerializer
      ).serializable_hash
      hash_to_render[:leads_in_pipeline] = leads_in_pipeline
      hash_to_render[:active_leads] = active_leads
      hash_to_render[:count] = @leads.count
      ...
      begin
        if @lead.update(lead_params)
          ...
          if @lead.lead_status == "delayed_possession" || @lead.lead_status == "delayed_project"
            @lead.events.where(status: ["scheduled", "rescheduled"]).update(status: "done")
            parsed_time = params[:lead][:follow_up_time].to_time.beginning_of_month + 11.hours
            if current_user.has_role?(:lead_head)
              create_event_for_follow_up(parsed_time, @lead.lead_status, @lead.user_type, @lead.id, [current_user.email])
            elsif current_user.has_role?(:cs_agent)
              emails = (User.with_role :lead_head).map(&:email)
              emails.push(current_user.email)
              create_event_for_follow_up(parsed_time, @lead.lead_status, @lead.user_type, @lead.id, emails)
            end
          end

          return render json: @lead, serializer: MobileLeadSerializer, status: 200
        else
          return render json: {message: @lead.errors.full_messages}, status: :unprocessable_entity
        end
      end
    end
  end
  ...
  def update_lead_status_from_landing_page
    ...
    if @lead.save!
      ...
      if @lead.note_records.blank?
        ...
        note_records = @lead.note_records.create(customer_name: @lead.name, phone: @lead.contact, project_name: "", city: params[:city], location: params[:location], scope_of_work: params[:scope_of_work], possession_status: possession_status_note_record, possession_status_date: possession_status_date_note_record, budget_value: params[:customer_budget], intended_date: project_start_date, home_type: home_type, project_type: params[:project_type], accomodation_type: params[:accomodation_type], additional_comments: params[:additional_comments])
  ...
  def update_lead_status
    ...
    if @lead.save!
      ...
      if @lead.note_records.blank?
        note_records = @lead.note_records.create(customer_name: @lead.name, phone: @lead.contact, project_name: "", city: params[:city], location: params[:location], scope_of_work: params[:scope_of_work], possession_status: possession_status_note_record, possession_status_date: possession_status_date_note_record, budget_value: params[:customer_budget], intended_date: params[:project_start_date].to_date, home_type: home_type, project_type: params[:project_type], accomodation_type: params[:accomodation_type], additional_comments: params[:additional_comments], society: params[:property_name], home_value: params[:home_value], have_homeloan: params[:have_home_loan], financial_solution_required: params[:financial_solution_required], have_floorplan: params[:have_floorplan], lead_floorplan: params[:lead_floorplan]&.first)
        note_records.scope_of_work.push scope_of_work_params
        note_records.save!
      else
        @lead.note_records.first.scope_of_work.push scope_of_work_params
  ...
  def update_status
    ...
    if status == "qualified"
      ...
      # create event only if the lead is already present - not when creating a new lead.
      ...
      create_event_for_follow_up(intended_time, "follow_up", @lead.user_type, @lead.id, [current_user.email]) if @lead.persisted? && intended_time.present?
  ...
  def assign_lead
    begin
      ...
      render json: {message: error.message.full_messages}, status: :unprocessable_entity
    end
  end
  ...
  def lead_pool_list
    unless current_user.has_any_role?(:admin, :lead_head, :cs_agent, :designer)
      return render json: {message: "You are not authorized to perform this action."}, status: :unauthorized
    end
  ...
  def smart_share_report
    ...
    when "lead_head"
      @leads = Lead.all
    when "designer"
      @leads = current_user.active_assigned_projects.pluck(:lead_id)
    when "city_gm"
  ...
  def lead_params
    params.require(:lead).permit("id", "name", "email", "contact", "address", "details", "created_at", 
      "updated_at", "city", "pincode", "user_type",
      "status_updated_at", "lead_status", "source", "follow_up_time", 
      "lost_remark", "lost_reason", "created_by", "lead_source_temp", "dummyemail", 
      "related_user_id", "lead_escalated", "reason_for_escalation", "lead_campaign_id", 
      "lead_source_id", "lead_type_id", "not_contactable_counter", "remark", "instagram_handle", 
      "referrer_id", "referrer_type", "is_in_pipeline")
  end

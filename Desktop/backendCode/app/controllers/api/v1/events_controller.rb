...
class Api::V1::EventsController < Api::V1::ApiController
    ...
    ActiveRecord::Base.transaction do
      begin
        unless params[:event][:scheduled_at] < Time.zone.now
          ...
          @event.update_attributes!(scheduled_at:params[:event][:scheduled_at],ownerable_id: params[:event][:ownerable_id], ownerable_type: params[:event][:ownerable_type])
          ...
          if @event.save!
            ...
            if @event.save!
              ...
              render json: {message: @event.errors.full_messages}, status: 422
            end
          else
  ...
  def change_event_status
    if @event.present?
      ...
      @event.review_location = params[:review_location]
    ...
    unless DateTime.parse(params[:event][:scheduled_at]) < Time.zone.now
      if @event.update!(event_params)
        if @event.status == "rescheduled"
          email_array = params[:event][:emails] ? params[:event][:emails] : params[:emails]
          @event.add_participants(email_array.split(";").map(&:strip), current_user.id)
          @event.users.each do |user|
            UserNotifierMailer.event_rescheduled_email(@event,user).deliver_later!(wait: 15.minutes)
            UserNotifierMailer.event_reminder(@event,user).deliver_later!(wait_until: @event.scheduled_at - 30.minute)
            SmsModule.delay(run_at: @event.scheduled_at - 30.minute).send_sms("You have a scheduled event in 30 minutes. Agenda: #{@event.agenda.humanize}, Scheduled at: #{@event.scheduled_at.strftime("%-d/%-m/%y: %I:%M %p")}", user.contact)
  ...
  def index
    ...
    @events = @events.where(scheduled_at: DateTime.parse(params[:from_date]).beginning_of_day..DateTime.parse(params[:to_date]).end_of_day)

    events = FjaEventSerializer.new(@events.includes(:users, :ownerable)).serializable_hash
    temp_hash = {}
  ...
  def events_for_ownerable
    if current_user.has_any_role?(:community_manager, :designer, :city_gm, :design_manager, :business_head)
      ...
      @events = Event.where(
        ownerable_id: params[:ownerable_id], ownerable_type: params[:ownerable_type],
  ...
  private
    ...
    def event_params
      params.require(:event).permit(
        :id, :agenda, :description, :scheduled_at, :status, :ownerable_type,
        :ownerable_id, :location, :contact_type, :remark,
        :review_location, :recording)
    end

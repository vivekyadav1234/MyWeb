class BoqForFinancePdf < Prawn::Document
	require 'open-uri'
  ...
  def boq_table(boq_table_details)
    ...
    discount_value = @quotation.discount_value_to_use
    flat_amount = @net_amount - (@net_amount * discount_value/100 ) 
    management_fee = (@quotation.nonservice_pm_fee.to_f + @quotation.service_pm_fee.to_f).round(2)
    total_amount = flat_amount + management_fee
	...
	def jobs_array_for_spaces(space)
		@srNo = 1
		space_table_head = ["Sr. No.", "Product", "Name", "Description", "BOQ Label","Rate", "Quantity", "Amount"]
		...
		final_array = []
		summary_array = []
		if furniture.present? || wardrobe.present? || kitchen_w.present? || kitchen_e.present? || kitchen_a.present? || custom.present? || service.present? || spaces_custom_furniture.present?
	...
	def lose_furnitures_for_space(space)
		...
		if loose_furnitures.present?
			loose_furnitures.each do |loose_furniture|
				...
				looseArray.push([@srNo,"Furniture", loose_furniture.name, description, loose_furniture.boq_labels.pluck("label_name").join(", "), loose_furniture.rate.round(2), loose_furniture.quantity, loose_furniture.amount])
				loosePrice += loose_furniture.amount
				@srNo += 1
				# image =""
				if @download_type.include?("annexure")
					begin
						image = {image: open("https:#{product.product_image.url}"), image_height: 150, image_width: 150, position: :center}
    ...
    if spaces_custom_furnitures.present?
      spaces_custom_furnitures.each do |spaces_custom_furniture|
        ...
        when 'cabinet'
          ...
          spaces_custom_furniture_array.push([@srNo,"Custom Furniture", custom_furniture_name, description, spaces_custom_furniture.boq_labels.pluck("label_name").join(", "), spaces_custom_furniture.cabinet_price_factored, spaces_custom_furniture.cabinet_quantity, spaces_custom_furniture.cabinet_amount_factored])
          @srNo += 1
        when 'door'
          spaces_custom_furniture_price += spaces_custom_furniture.amount_factored if spaces_custom_furniture.amount_factored.present?
          ...
          "\n Core Material: #{spaces_custom_furniture.get_core_material}"
          spaces_custom_furniture_array.push([@srNo, "Custom Furniture",custom_furniture_name, description, spaces_custom_furniture.boq_labels.pluck("label_name").join(", "), spaces_custom_furniture.door_price_factored, spaces_custom_furniture.door_quantity, spaces_custom_furniture.door_amount_factored])
          @srNo += 1
        when 'accessory'
          spaces_custom_furniture_price += spaces_custom_furniture.amount_factored if spaces_custom_furniture.amount_factored.present?
          ...
          " \n Material: #{spaces_custom_furniture&.job_material}" +
          "\n Core Material: #{spaces_custom_furniture.get_core_material}"
          spaces_custom_furniture_array.push([@srNo,"Custom Furniture", custom_furniture_name,description, spaces_custom_furniture.boq_labels.pluck("label_name").join(", "), spaces_custom_furniture.accessory_price_factored, spaces_custom_furniture.accessory_quantity, spaces_custom_furniture.accessory_amount_factored])
          @srNo += 1
        when 'sliding_door'
          spaces_custom_furniture_price += spaces_custom_furniture.amount_factored if spaces_custom_furniture.amount_factored.present?
          item = spaces_custom_furniture&.door_item ? spaces_custom_furniture&.door_item : spaces_custom_furniture.door_style_code 
        ...
        when 'wardrobe'
          spaces_custom_furniture_price += spaces_custom_furniture.amount_factored if spaces_custom_furniture.amount_factored.present?
          spaces_custom_furniture_array.push([@srNo,"Custom Furniture", job.capitalize,"", spaces_custom_furniture.boq_labels.pluck("label_name").join(", "),spaces_custom_furniture.wardrobe_price_factored, "", spaces_custom_furniture.wardrobe_amount_factored])
          @srNo += 1
        else
          nil  
        end           
    ...
    if wardrobe_jobs.present?
      wardrobe_jobs.each do |wardrobe_job|
        if !wardrobe_job.combined_module_id.present? || wardrobe_job.combined
          ...
          wardrobeArray.push([@srNo,"Wardrobe", wardrobe_name, description, wardrobe_job.boq_labels.pluck("label_name").join(", "), wardrobe_job.rate.round(2), wardrobe_job.quantity, wardrobe_job.amount])
  ...
  def kitchen_modulor_job(space, kitchen_type)
    ...
    if kitchen_jobs.present?
        ...
        kitchenArray.push([@srNo,kitchen_type, wardrobe_name, description, kitchen_job.boq_labels.pluck("label_name").join(", "), kitchen_job.rate.round(2), kitchen_job.quantity, kitchen_job.amount])
  ...
  def kitchen_extra_job(space, category)
    ...
    if kitchen_jobs.present?
      kitchen_jobs.each do |kitchen_job|
        ...
        kitchenArray.push([@srNo,"#{category.humanize}" + " Extra", kitchen_job&.name, description, kitchen_job.boq_labels.pluck("label_name").join(", "), kitchen_job.rate.round(2), kitchen_job.quantity, kitchen_job.amount])
        kitchenPrice += kitchen_job.amount
        @srNo += 1
      end
      {space_section_table: kitchenArray, space_section_price: kitchenPrice}
	...
	def kitchen_appliance_job(space)
		...
		if kitchen_jobs.present?
			kitchen_jobs.each do |kitchen_job|
				...
				kitchenArray.push([@srNo,"Appliance", kitchen_job&.name, description, kitchen_job.boq_labels.pluck("label_name").join(", "), kitchen_job.rate.round(2), kitchen_job.quantity, kitchen_job.amount])
				kitchenPrice += kitchen_job.amount
				@srNo += 1
			end
			{space_section_table: kitchenArray, space_section_price: kitchenPrice}
	...
	def service_for_space(space)
		...
		if service_jobs.present?
			...
			price_factor_hash =  service_job.ownerable.get_price_factor_hash
    		description = "Base Price #{service_job&.base_rate&.round(2)} INR\n Installation Rate : #{service_job&.installation_rate&.round(2)} INR"
				serviceArray.push([@srNo,"Service", service_job&.name, description, "", (service_job.final_rate&.round(2) * service_job.get_price_increase_factor)&.round(2), service_job.quantity, service_job.amount])
				@srNo += 1
			end
			serviceArray
		else

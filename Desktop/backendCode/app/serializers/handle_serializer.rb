...
class HandleSerializer < ActiveModel::Serializer
  attributes :id, :code, :handle_type, :price, :handle_image, :created_at, :updated_at, :category, 
  	:mrp, :vendor_sku, :spec, :make, :unit, :arrivae_select, :handle_class, :hidden, :modspace

  attribute :image_present

  def image_present
  	object.handle_image_file_name.present?
  end
end

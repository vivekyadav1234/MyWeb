...
class DesignerBookingFormSerializer < ActiveModel::Serializer
  attribute :general
  attribute :modular_kitchen

  def general
    hash = Hash.new
    ["id", "customer_name", "customer_age", "profession", "family_profession", "age_house", 
      "lifestyle", "house_positive_features", "house_negative_features", "inspiration", 
      "color_tones", "theme", "functionality", "false_ceiling", "electrical_points", "special_needs", 
      "vastu_shastra", "all_at_once", "budget_range", "design_style_tastes", "storage_space", 
      "mood", "enhancements", "discuss_in_person", "project_id", "created_at", "updated_at"].each do |k|
        hash[k] = object.public_send(k)
        hash["inspiration_image"] = object.inspiration_image&.url
    end
    hash
  end
  ...
  def modular_kitchen
    hash = Hash.new
    ["id","mk_age", "mk_gut_kitchen", "mk_same_layout", "mk_improvements", "mk_special_requirements", 
    ...
    "project_id", "created_at", "updated_at"].each do |k|
        hash[k] = object.public_send(k)
        hash["mk_lifestyle"] = object.lifestyle   #Same as :lifestyle column in key :general
    end
    hash
  end
end
